...
def max_pooling(img_array,filter_width=2,filter_height=2,stride=1):
    ...
    for i in range(out_h):
        for j in range(out_w):
            segment = img_array[i:i+filter_height,j:j+filter_width]
...
if __name__ == "__main__":
    ...
    # print(img_array)
    pooled_array = max_pooling(img_array)
    # print(pooled_array)
    ...
    for i in range(layers):
        ...
        pooled_array_uint8 = pooled_array.astype(np.uint8)  # Convert to 8-bit unsigned int
        pooled_img = Image.fromarray(pooled_array_uint8)
        pooled_img.save(f"max_pooled_{i}.png")
        img_array = pooled_array_uint8